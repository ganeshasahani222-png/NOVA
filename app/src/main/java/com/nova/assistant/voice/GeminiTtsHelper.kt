 ...
 * Playback is synchronous (blocks until audio finishes) so callers
 * know it's safe to resume listening afterwards without the mic
 * picking up Nova's own voice.
 */
class GeminiTtsHelper(private val context: Context) {
    ...
    private fun playPcmAudio(pcmData: ByteArray) {
        ...
        val durationMillis = (pcmData.size.toLong() / bytesPerSample) * 1000 / sampleRate
        Thread.sleep(durationMillis + 200)
